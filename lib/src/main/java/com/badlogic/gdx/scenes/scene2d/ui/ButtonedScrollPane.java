...
/**
 * A group that scrolls a child widget using scrollbars. Unlike the regular {@link ScrollPane}
 * implementation, this variant does not support mouse/touch dragging. Additionally, this version
 * includes the support for scroll buttons at the top and bottom of the scroll background.
 * <p>
 ...
 * @author Sino
 */
public final class ButtonedScrollPane extends WidgetGroup {
    private SimpleScrollPaneStyle style;
    ...
    boolean smoothScrolling = true, scrollBarTouch = true;
    ...
    private Button hScrollButton, vScrollButton;

    private Drawable curVScrollKnob;
    private Drawable curHScrollKnob;

    /**
     * @param widget May be null.
     */
    public ButtonedScrollPane(Actor widget) {
        this(widget, new SimpleScrollPaneStyle());
    }

    /**
     * @param widget May be null.
     */
    public ButtonedScrollPane(Actor widget, Skin skin) {
        this(widget, skin.get(SimpleScrollPaneStyle.class));
    }

    /**
     * @param widget May be null.
     */
    public ButtonedScrollPane(Actor widget, Skin skin, String styleName) {
        this(widget, skin.get(styleName, SimpleScrollPaneStyle.class));
    }

    /**
     * @param widget May be null.
     */
    public ButtonedScrollPane(Actor widget, SimpleScrollPaneStyle style) {
        if (style == null) throw new IllegalArgumentException("style cannot be null.");
        this.style = style;

        if (style.hScrollButton != null) hScrollButton = new Button(style.hScrollButton);
        if (style.vScrollButton != null) vScrollButton = new Button(style.vScrollButton);

        if (style.vScrollKnob != null) curVScrollKnob = style.vScrollKnob.up;
        if (style.hScrollKnob != null) curHScrollKnob = style.hScrollKnob.up;
        ...
        addCaptureListener(new InputListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (getStage() != null) getStage().setScrollFocus(ButtonedScrollPane.this);

                if (scrollBarTouch && scrollX && hScrollBounds.contains(x, y)) {
                    event.stop();

                    if (hKnobBounds.contains(x, y)) {
                        if (style.hScrollKnob != null) curHScrollKnob = style.hScrollKnob.down;
                ...
                if (scrollBarTouch && scrollY && vScrollBounds.contains(x, y)) {
                    ...
                    if (vKnobBounds.contains(x, y)) {
                        if (style.vScrollKnob != null) curVScrollKnob = style.vScrollKnob.down;
    ...
    public void setStyle(SimpleScrollPaneStyle style) {
        if (style == null) throw new IllegalArgumentException("style cannot be null.");
        this.style = style;
        invalidateHierarchy();
    }
     ...
     * {@link #setStyle(SimpleScrollPaneStyle)} is called.
     */
    public SimpleScrollPaneStyle getStyle() {
        return style;
    }
    ...
    public void layout() {
        ...
        final Drawable hScrollKnob = curHScrollKnob;
        final Drawable vScrollKnob = curVScrollKnob;
    ...
    private void updateWidgetPosition() {
        ...
        if (scrollbarsOnTop) {
            if (scrollX && hScrollOnBottom) {
                float scrollbarHeight = 0;
                if (style.hScrollKnob != null) scrollbarHeight = curHScrollKnob.getMinHeight();
                if (style.hScroll != null) scrollbarHeight = Math.max(scrollbarHeight, style.hScroll.getMinHeight());
                y += scrollbarHeight;
            }
            if (scrollY && !vScrollOnRight) {
                float scrollbarWidth = 0;
                if (style.hScrollKnob != null) scrollbarWidth = curHScrollKnob.getMinWidth();
    ...
    protected void drawScrollBars(Batch batch, float r, float g, float b, float a) {
        ...
        if (x) {
            ...
            if (curHScrollKnob != null)
                curHScrollKnob.draw(batch, hKnobBounds.x, hKnobBounds.y, hKnobBounds.width, hKnobBounds.height);
        }
        if (y) {
            if (style.vScroll != null)
                style.vScroll.draw(batch, vScrollBounds.x, vScrollBounds.y, vScrollBounds.width, vScrollBounds.height);

            if (curVScrollKnob != null)
                curVScrollKnob.draw(batch, vKnobBounds.x, vKnobBounds.y, vKnobBounds.width, vKnobBounds.height);
        }
    }
    ...
    public float getPrefWidth() {
        ...
        if (scrollY) {
            float scrollbarWidth = 0;
            if (curVScrollKnob != null) scrollbarWidth = curVScrollKnob.getMinWidth();
            if (style.vScroll != null) scrollbarWidth = Math.max(scrollbarWidth, style.vScroll.getMinWidth());
    ...
    public float getPrefHeight() {
        ...
        if (scrollX) {
            float scrollbarHeight = 0;
            if (curHScrollKnob != null) scrollbarHeight = curHScrollKnob.getMinHeight();
            if (style.hScroll != null) scrollbarHeight = Math.max(scrollbarHeight, style.hScroll.getMinHeight());
    ...
    public float getScrollBarHeight() {
        if (!scrollX) return 0;
        float height = 0;
        if (curHScrollKnob != null) height = curHScrollKnob.getMinHeight();
    ...
    public float getScrollBarWidth() {
        if (!scrollY) return 0;
        float width = 0;
        if (curVScrollKnob != null) width = curVScrollKnob.getMinWidth();
    ...
    /**
     * When false, the scroll bars don't respond to mouse events. Default is true.
     */
     ...
     * to the entire scroll pane bounds and the scrollbars are drawn on top of the widget.
     */
    ...
    /**
     * The style for a scroll pane, see {@link ButtonedScrollPane}.
     *
     * @author mzechner
     * @author Nathan Sweet
     */
    static public class SimpleScrollPaneStyle {
        /**
         * Optional.
         */
         ...
         * Optional.
         */
        public Button.ButtonStyle hScrollKnob;
        /**
         * Optional.
         */
         ...
         * Optional.
         */
        public Button.ButtonStyle vScrollKnob;
        /**
         * Optional.
         */
        public Button.ButtonStyle hScrollButton, vScrollButton;

        public SimpleScrollPaneStyle() {
        }

        public SimpleScrollPaneStyle(Drawable background, Drawable hScroll, Button.ButtonStyle hScrollKnob, Drawable vScroll,
                                     Button.ButtonStyle vScrollKnob, Button.ButtonStyle hScrollButton, Button.ButtonStyle vScrollButton) {
            this.background = background;
            this.hScroll = hScroll;
            this.hScrollKnob = hScrollKnob;
            this.vScroll = vScroll;
            this.vScrollKnob = vScrollKnob;
            this.hScrollButton = hScrollButton;
            this.vScrollButton = vScrollButton;
        }

        public SimpleScrollPaneStyle(SimpleScrollPaneStyle style) {
            ...
            this.hScrollButton = style.hScrollButton;
            this.vScrollButton = style.vScrollButton;
        }
    }
}
